...
async function positionToLocation(location) {
  try {
    ...
    const fetchUrl = `https://maps.googleapis.com/maps/api/geocode/json?latlng=${latitude},${longitude}&key=${GOOGLE_API_KEY}&language=ko`;
...
async function mergeLocationsData(capitalLocationData, locationData) {
  const matchedPlace = capitalLocationData.filter(
    (capital) =>
      capital.administrativeAreaKorean === locationData.administrativeArea
  );
  const matchedLocation = matchedPlace.length > 0 ? matchedPlace[0] : {};
  const administrativeArea = matchedLocation.administrativeArea;
  const { convertedX, convertedY, formattedAddress } = locationData;

  return {
    administrativeArea,
    administrativeAreaKorean: locationData.administrativeArea,
    ...matchedLocation,
    convertedX,
    convertedY,
    formattedAddress,
